 ...
 * \file io_halos.c
 *
 * Provides functions for reading and writing AHF_HALOS files.
 */
...
#define LOCAL_HALOS_BUFFERSIZE 2048
 ...
 * io_halos_open more readable.
 *
 * \param log   The logging object.
 * \param f     The AHF_HALOS file object sofar
 * \param mode  The mode in which to open the file.
 *
 * \returns In case of an error NULL is returned, otherwise f.
 */
inline static io_halos_t
local_openopen(io_logging_t log, io_halos_t f, io_file_mode_t mode);
...
\**********************************************************************/
extern io_halos_t
io_halos_open(io_logging_t log,
              char *fname,
              io_file_mode_t mode,
              uint32_t reader)
{
	...
	f = (io_halos_t)malloc(sizeof(io_halos_struct_t));
	if (f == NULL) {
		io_logging_memfatal(log,  "io_halos structure");
		return NULL;
	}
	...
	if (f->fname == NULL) {
		io_logging_memfatal(log, "filename of AHF_HALOS file");
		free(f);
		return NULL;
	}
	strncpy(f->fname, fname, strlen(fname)+1);

	/* Okay, we are an AHF_HALOS file */
	f->ftype = IO_FILE_HALOS;
...
extern void
io_halos_close(io_logging_t log,
               io_halos_t *f)
{
	/* Catch NULLs */
	if (f == NULL || *f == NULL)
	...
	if (    ((*f)->mode == IO_FILE_WRITE)
	     && ((*f)->header != NULL)) {
		io_halos_header_write(log, (*f)->header, *f);
	}
	...
	/* Clean up */
	if ((*f)->header != NULL)
		io_halos_header_del(log, &((*f)->header));
	if ((*f)->fname != NULL)
		free((*f)->fname);
#	ifdef WITH_MPI
...
extern void
io_halos_init(io_logging_t log,
              io_halos_t f)
{
	if (f->mode == IO_FILE_READ) {
		io_logging_msg(log, INT32_C(5),
		               ...
		               f->fname);
		f->header = io_halos_header_get(log, f);
		io_logging_msg(log, INT32_C(5),
...
extern uint64_t
io_halos_readpart(io_logging_t log,
                  io_halos_t f,
                  uint64_t pskip,
                  uint64_t pread,
                  io_file_strg_struct_t strg)
{
	...
	char buffer[LOCAL_HALOS_BUFFERSIZE];
	...
	rewind(f->file);
	do {
		fgets(buffer, LOCAL_HALOS_BUFFERSIZE, f->file);
	} while (buffer[0] == '#');
	for (i=0; i<pskip; i++)
		fgets(buffer, LOCAL_HALOS_BUFFERSIZE, f->file);
	...
	for (i=0; i<pread; i++) {
     ...
     \**************************************************/
#ifdef NON_AHF_CATALOGUE
    
    sscanf(buffer, "%f %f %f %lf %lf %lf %lf %lf %lf %lf",
           &dummy,&dummy,&dummy,
           &fweight,
           &dummy,
           &fposx, &fposy, &fposz,
...
#else // NON_AHF_CATALOGUE
    
		if (f->header->multi_mass == 1) {
			sscanf(buffer, "%f %f %f %lf %f %lf %lf %lf %lf %lf %lf",
             &dummy,&dummy,&dummy,
             &fweight,
             &dummy,
			       &fposx, &fposy, &fposz,
...
extern uint64_t
io_halos_writepart(io_logging_t log,
                   io_halos_t f,
                   uint64_t pskip,
                   uint64_t pwrite,
                   io_file_strg_struct_t strg)
...
extern uint64_t
io_halos_writepart_ord(io_logging_t log,
                       io_halos_t f,
                       uint64_t pskip,
                       uint64_t pwrite,
                       void *nxt_part,
             ...
             void *res)
{
	...
	if (f->header == NULL) {
		io_logging_warn(log, INT32_C(3),
		                ...
		                "creating one.");
		f->header = io_halos_header_new(log);
		if (f->header == NULL)
			return false;
	}
...
extern void
io_halos_log(io_logging_t log, io_halos_t f)
{
	io_logging_msg(log, INT32_C(5),
	...
	io_logging_msg(log, INT32_C(5),
	               "  maxweight:         %g",
	               f->maxweight);
	io_halos_header_log(log, f->header);
