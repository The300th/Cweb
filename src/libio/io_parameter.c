...
/*--- Local variables ---------------------------------------------------*/
static const char *local_secName = "CWEB";
...
//	           ini, "NperRefCell", local_secName);
  params->Nth_ref = 10000;
	...
	getFromIni(&(params->RSMOOTH), parse_ini_get_double,
	           ini, "Rsmooth", local_secName);
  
	 /* All these are not needed by Cweb */
	 params->UseRhoBack   = -1;
	 params->UserDvir     = -1;
	 params->MaxGatherRad = -1;
	 params->AHF_MINPART  = -1;
  ...
  if(params->ic_filetype == IO_FILE_ART)
    ...
    *    AHF_HALOS
    *-----------------*/
   if(params->ic_filetype == IO_FILE_HALOS)
    {
     FILE   *fpout;
     double HALOS_BOXSIZE, HALOS_REDSHIFT;
     uint64_t HALOS_N;
     
     getFromIni(&HALOS_N, parse_ini_get_uint64,
                ini, "HALOS_N", "AHF_HALOS");
     getFromIni(&HALOS_BOXSIZE, parse_ini_get_double,
                ini, "HALOS_BOXSIZE", "AHF_HALOS");
     getFromIni(&HALOS_REDSHIFT, parse_ini_get_double,
                ini, "HALOS_REDSHIFT", "AHF_HALOS");
     
 #ifdef VERBOSE
     fprintf(stderr,"HALOS_N             = %"PRIu64"\n",HALOS_N);
     fprintf(stderr,"HALOS_BOXSIZE       = %20.10lf\n",HALOS_BOXSIZE);
     fprintf(stderr,"HALOS_REDSHIFT      = %20.10lf\n",HALOS_REDSHIFT);
     fprintf(stderr,"\n");
 #endif
     ...
     if(global_mpi.rank == 0) // only the master should write the file
      {
 #endif
       if( (fpout = fopen("ahf_halos.info","w")) == NULL)
        {
         fprintf(stderr,"Could not open ahf_halos.info\nAborting\n");
         exit(0);
        }
       fprintf(fpout,"%"PRIu64" \t\t HALOS_N\n",HALOS_N);
       fprintf(fpout,"%20.10lf \t\t HALOS_BOXSIZE\n",HALOS_BOXSIZE);
       fprintf(fpout,"%20.10lf \t\t HALOS_REDSHIFT\n",HALOS_REDSHIFT);
       fclose(fpout);
 #ifdef WITH_MPI
      }
 #endif
