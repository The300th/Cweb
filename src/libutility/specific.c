...
void binning_parameter(HALO halo, int *nbins, double *dist_min, double *dist_max)
{
   ...
   *nbins   = MAX(1,*nbins) * AHF_NBIN_MULTIPLIER;
   ...
   /* minimum distance */
   npart = 0;
   while(npart < halo.npart && npart < simu.AHF_MINPART/10.)
     {
        npart++;
...
void write_logfile(double timecounter, double timestep, int no_timestep)
{
  ...
  for(for_grid = global.dom_grid; for_grid->l1dim < global.fin_l1dim; for_grid++)
   {
    fprintf(io.logfile,"GRID %12ld: nodes=%12u (%8.3g GB) npart=%12u - TIME: pot=%6ld dens=%6ld DK=%6ld grid=%6ld hydro=%6ld - SWEEPS: %ld %g\n",
            for_grid->l1dim,
  ...
  /* write grid information to logfile */
  /*-----------------------------------*/
  fprintf(io.logfile,"GRID %12ld: nodes=%12u (%8.3g GB) npart=%12u - TIME: pot=%6ld dens=%6ld grid=%6ld\n",
          for_grid->l1dim,
          for_grid->size.no_nodes, for_grid->size.no_nodes * global.bytes_node * bytes2GB,
          for_grid->size.no_part,
          for_grid->time.potential,
...
//       }
      ...
      if( (fpparam = fopen(param_file,"w")) == NULL)
      ...
      else
       {
...
#endif
#ifdef AHFpreliminaryhalosfile
        fprintf(fpparam,"AHFpreliminaryhalosfile    \t\t1\n");
#else
        fprintf(fpparam,"AHFpreliminaryhalosfile    \t\t0\n");
#endif
        ...
        fprintf(fpparam,"--------------\n");
#ifdef LAMBDA_TH
        fprintf(fpparam,"LAMBDA_TH=%g\t\t1\n",LAMBDA_TH);
#endif
#ifdef HUBBLEZ
         fprintf(fpparam,"HUBBLEZ                   \t\t1\n");
#else
...
//         fprintf(fpparam,"GRID_redshift=%g\n",GRID_redshift);
//         fprintf(fpparam,"GRID_boxsize=%g\n",GRID_boxsize);
//         fprintf(fpparam,"GRID_omega0=%g\n",GRID_omega0);
//         fprintf(fpparam,"GRID_lambda0=%g\n",GRID_lambda0);
#else
         fprintf(fpparam,"READ_GRIDDATA                        \t\t0\n");
#endif
#ifdef WRITE_GRIDDATA
        fprintf(fpparam,"WRITE_GRIDDATA                        \t\t1\n");
#else
        fprintf(fpparam,"WRITE_GRIDDATA                        \t\t0\n");
#endif
        ...
        fprintf(fpparam,"-----------------------------\n");
#ifdef USE_FFTW3
        fprintf(fpparam,"USE_FFTW3                  \t\t1\n");
#else
        fprintf(fpparam,"USE_FFTW3                  \t\t0\n");
#endif
#ifdef FIXED_SWEEPS
...
/*==============================================================================
 * small routine calculating cNFW according to Eq.(8) in Prada et al. (2012)
