...
{
  double a2, a3, rho_crit, omega0, lambda0;
...
#ifdef DVIR_200RHOCRIT_VDE
  rho_crit = 3.*pow2(calc_Hubble_VDE(a))/8./PI/Grav;
#else
...
double calc_virial(double a)
{
  ...
  if(simu.UserDvir > 0)
  ...
  else
   {
    ...
    eta      = (double)2.*simu.lambda0/omega_ta/pow3(a);
    ...
    /* check whether we want virial in terms of RhoBack or RhoCrit */
    if(simu.UseRhoBack == TRUE)
      virial = pow3(a) * omega_ta/simu.omega0/pow3(reduce);
    else
      virial = pow3(a) * omega_ta/pow3(reduce);
...
void create_timeline(double a_init, double a_fin, tlptr timeline)
{
  ...
  fprintf(stderr,"   creating timeline from z=%lf to z=%lf in %d points...",1/a_init-1,1/a_fin-1,MAXTIME);
