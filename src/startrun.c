...
#ifdef VERBOSE
  fprintf(stderr,"startrun: Returning to main()\n");
#endif
...
local_startrunLog(void)
{
  ...
  io_logging_start(global_io.params->outfile_prefix,
	...
	io_logging_msg(global_io.log, INT32_C(2), "Rsmooth           = %g [Mpc/h]", global_io.params->RSMOOTH);
...
local_startrunRead(void)
{
	...
	global_info.no_part  = io_file_get_numpart(global_io.log, global_io.file, &pskip, &pread);
...
#	ifdef METALHACK
	...
	strg.z.stride =   (char *)&((global_info.fst_part+1)->z) - (char *)&(global_info.fst_part->z);
	strg.age.val = &(global_info.fst_part->age);
	strg.age.stride =   (char *)&((global_info.fst_part+1)->age) - (char *)&(global_info.fst_part->age);
#	endif
...
#	endif
    ...
    io_logging_msg(global_io.log, INT32_C(5), "simu.lb_level        :  %g", simu.lb_level);
