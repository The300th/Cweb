...
boolean assign_dens(gridls *cur_grid)
{
  ...
  int iseed=123456;
   ...
   double  mass2dens, vel2momdens, ratio, cur_shift;
                     ...
                     for(cur_part = cur_node->ll; cur_part != NULL; cur_part = cur_part->ll)
                       {
...
#endif
                           cur_grid->masstodens;        // masstodens = l1dim^3/MtotInSimu, ie normalisation to background density
...
#ifdef UonGrid
                                    tsc_nodes[k][j][i]->u     += cur_part->u * weights[k][Z]*weights[j][Y]*weights[i][X];
#endif
                                   }
                              } } } 
                        ...
                        for(idim=0; idim<NDIM; idim++)
                         {
                          // mom[] stores a^2 \dot{x} and mass2dens converts to momentum density
                          vel2momdens = cur_part->mom[idim] * mass2dens;
                          ...
                          for(k = 0; k < 3; k++){
                            for(j = 0; j < 3; j++){
                              for(i = 0; i < 3; i++){
                                if(tsc_nodes[k][j][i] != NULL)
                                 {
                                  tsc_nodes[k][j][i]->densV[idim] += vel2momdens * weights[k][Z]*weights[j][Y]*weights[i][X];
                                 } 
                              } } } 
                         } /* idim */
               ...
               for(icur_nquad=cur_nquad;icur_nquad!=NULL;icur_nquad=icur_nquad->next)
                  ...
                  for(cur_node = icur_nquad->loc; 
                      ...
                      cur_node++)
                    {
                     ...
                     cur_node->densV[X]      = 0.0;
                     cur_node->densV[Y]      = 0.0;
                     cur_node->densV[Z]      = 0.0;
...
void refill_dens(gridls *coa_grid)
{
   ...
   /* re-calculate P3 density field on fine grid (fin_node->dens) */
   zero_dens(fin_grid);
   assign_dens(fin_grid);
   
   /* store (P1/P2) density spilling from coarse to fine grid (coa_node->dens -> fin_node->force.temp[0]) */
   fc_overlap(fin_grid);
   
   /* interpolate P3 density from fine to coarse grid (fin_node->dens -> coa_node->dens) */
   f2c_dens(fin_grid);
   
   /* add stored P1/P2 contribution to already existing P3 density (fin_node->dens += fin_node->force.temp[0]) */
